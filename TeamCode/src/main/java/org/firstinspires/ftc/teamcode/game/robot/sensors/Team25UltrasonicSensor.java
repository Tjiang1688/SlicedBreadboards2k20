package org.firstinspires.ftc.teamcode.game.robot.sensors;
    ...
    public void portIsReady(int port)
    {
        ...
        try {
            if (transaction.write) {
                legacyModule.enableI2cWriteMode(port, new I2cAddr(I2C_ADDRESS), transaction.offset, transaction.len);
                legacyModule.copyBufferIntoWriteBuffer(port, transaction.buffer);
                transaction.state = I2cTransactionState.PENDING_I2C_WRITE;
            } else {
                legacyModule.enableI2cReadMode(port, new I2cAddr(I2C_ADDRESS), transaction.offset, transaction.len);
